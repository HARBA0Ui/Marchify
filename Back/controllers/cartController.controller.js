  ...
  try {
    ...
    const commande = await db.commande.create({
      data: {
        ...
        adresseLivraison,
        totalCommande,
        boutiqueId, // assigner si tous les produits sont de la même boutique
        produits: {
          create: panier.produits.map((p) => ({
            quantite: p.quantite,
            prixTotal: p.prixTotal,
            produit: { connect: { id: p.produitId } }, // important pour MongoDB
            boutique: { connect: { id: p.produit.boutiqueId } } // assigner boutique à chaque produit
          }))
        }
      },
      include: { produits: true }
